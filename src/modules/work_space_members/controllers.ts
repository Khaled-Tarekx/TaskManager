...
export const getMembersOfWorkSpace = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
...
export const createMember = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
    const {role} = req.body
...
export const getMember = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
...
export const updateMemberPermissions = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
    ...
    const {role} = req.body
    const updatedMember = await WorkSpaceMembers.findOneAndUpdate({id, workspaceId},
        {role},
        {new: true, runValidators: true, context: 'query'});
    if (!updatedMember) return next(new NotFound(`no member found with the given id`))
    res.status(StatusCodes.OK).json({message: 'member updated Successfully', data: updatedMember})
});

export const deleteMember = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
    const {id, workspaceId} = req.params
    if (!mongoose.Types.ObjectId.isValid(id)) {
        return next(new BadRequest('Invalid ID format'))
    }
