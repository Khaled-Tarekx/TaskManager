...
export const createTask = asyncHandler(
	async (
		...
		const loggedInUser = await checkUser(user);
		const worker = await findResourceById(Member, workerId);
		const creator = await findResourceById(Member, loggedInUser.id);
		if (
			worker.workspace.id !== workspaceId ||
			creator.workspace.id !== workspaceId
		) {
			return next(
		...
		const task = await Task.create({
			...
			worker: worker.id,
			workspace: creator.workspace.id,
			dead_line,
			dependants,
			priority,
			skill_set,
...
export const assignTask = asyncHandler(
	async (req: Request, res: Response, next: NextFunction) => {
		...
		validateObjectIds([taskId, user_id]);
		const loggedInUser = await checkUser(user);
		const task = await findResourceById(Task, taskId);
		await isResourceOwner(loggedInUser.id, task.creator.id);
		...
		const assignedTask = await Task.findByIdAndUpdate(
			task.id,
			{ worker: user_id, status: 'inProgress' },
			{ new: true }
		).populate({
			path: 'worker',
